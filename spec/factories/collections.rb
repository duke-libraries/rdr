# NOTE: Copied from Hyrax core (v2.6.0) for local RDR use & extension
...
FactoryBot.define do
  ...
  # DUL CUSTOM: Build a collection using custom RDR Collection Type
  factory :rdr_collection, class: Collection, traits: [:public_rdr] do
    transient do
      user { create(:user) }
    end

    sequence(:title) { |n| ["RDR Collection Title #{n}"] }

    after(:build) do |collection, evaluator|
      collection.apply_depositor_metadata(evaluator.user.user_key)
      collection_type = create(:rdr_collection_type)
