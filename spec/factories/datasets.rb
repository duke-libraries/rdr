FactoryBot.define do
  factory :dataset do
    ...
    factory :dataset_with_one_child do
      before(:create) do |work, evaluator|
        work.ordered_members << FactoryBot.create(:dataset, user: evaluator.user, title: ['A Contained Work'])
      end
    end
    ...
    factory :dataset_with_file_and_work do
      before(:create) do |work, evaluator|
        work.ordered_members << FactoryBot.create(:file_set, user: evaluator.user)
        work.ordered_members << FactoryBot.create(:dataset, user: evaluator.user)
      end
    end
