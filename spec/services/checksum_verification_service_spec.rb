...
RSpec.describe ChecksumVerificationService do

  let(:wrapper) { double('JobIOWrapper') }

  describe '.call' do
    let(:notification) { [ Rdr::Notifications::FILE_INGEST_FINISHED, Date.new, Date.new, 'test', wrapper: wrapper ] }
    it 'enqueues a ChecksumVerificationJob' do
      expect(ChecksumVerificationJob).to receive(:perform_later).with(wrapper)
  ...
  describe '#verify_checksum' do
    ...
    describe 'fileset files' do
      ...
      describe 'files not present' do
        it 'causes an error' do
          expect { subject.verify_checksum }.
              to raise_error(Rdr::ChecksumInvalid, I18n.t('rdr.fileset_without_files', fs_id: file_set.id))
        end
      end
    end
